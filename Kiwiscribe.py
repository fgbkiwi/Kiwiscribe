...
def load_api_keys():
    ...
    keys = {'assembly_ai': '', 'claude': '', 'openai': '', 'gemini': '', 'soniox': ''} # Padrão
...
def save_api_keys(assembly_key, claude_key, openai_key, gemini_key, soniox_key):
...
def extract_info_from_ata(ata_path):
    ...
    try:
        ...
        recl_pattern1 = r'Presente a parte reclamante ([^,]+), pessoalmente, acompanhado\(?a?\)? de seu\(?a?\)? advogado\(?a?\), Dr\(?a?\)\. ([^,]+), OAB'
        ...
        if not reclamante_encontrado:
            recl_pattern2 = r'Presente a parte reclamante ([^,]+),.*?Dr\(?a?\)\. ([^,]+), OAB'
        ...
        if not reclamante_encontrado:
            ...
            reclamante_match = re.search(recl_pattern3, text, re.IGNORECASE | re.DOTALL)
            if reclamante_match:
        ...
        # Reclamada(s), Preposto(s) e Advogado(s): PADRÕES SIMPLIFICADOS E EFICIENTES
        reclamada_encontrada = False
        
        # Padrão 1: "Presente a parte reclamada NOME, representado(a) pelo(a) preposto(a) Sr.(a) NOME, acompanhado(a) de seu(a) advogado(a), Dr(a). NOME, OAB"
        reclamada_pattern1 = r'Presente a parte reclamada ([^,]+), representado\(?a?\)? pelo\(?a?\)? preposto\(?a?\) Sr\(?\.?\)?\(?a?\)? ([^,]+), acompanhado\(?a?\)? de seu\(?a?\)? advogado\(?a?\), Dr\(?a?\)\. ([^,]+), OAB'
        match_reclamada = re.search(reclamada_pattern1, text, re.IGNORECASE)
        if match_reclamada:
            print(f"Reclamada encontrada (padrão 1).")
            reclamada_encontrada = True
            nome_reclamada = ' '.join(match_reclamada.group(1).strip().split()).title()
            nome_preposto = ' '.join(match_reclamada.group(2).strip().split()).title()
            nome_advogado = ' '.join(match_reclamada.group(3).strip().split()).title()
            
            info['reclamadas'].append(nome_reclamada)
            info['prepostos'].append(nome_preposto)
            info['advogados_reclamadas'].append(nome_advogado)
            print(f"1ª Reclamada: {nome_reclamada}, Preposto: {nome_preposto}, Adv: {nome_advogado}")
        
        # Padrão 2: "Presente a parte reclamada NOME, ... Dr(a). NOME, OAB" (mais flexível)
        if not reclamada_encontrada:
            reclamada_pattern2 = r'Presente a parte reclamada ([^,]+),.*?Dr\(?a?\)\. ([^,]+), OAB'
            match_reclamada = re.search(reclamada_pattern2, text, re.IGNORECASE | re.DOTALL)
            if match_reclamada:
                print(f"Reclamada encontrada (padrão 2).")
                reclamada_encontrada = True
                nome_reclamada = ' '.join(match_reclamada.group(1).strip().split()).title()
                ...
                info['advogados_reclamadas'].append(nome_advogado)
                print(f"1ª Reclamada: {nome_reclamada}, Preposto: N/I, Adv: {nome_advogado}")
        
        # Padrão 3: "RECLAMADO: NOME ADVOGADO: NOME" (folha de rosto)
        if not reclamada_encontrada:
            reclamada_pattern3 = r'RECLAMADO\s*:\s*([^,\n]+?)\s*ADVOGADO\s*:\s*([^,\n]+)'
            matches = list(re.finditer(reclamada_pattern3, text, re.IGNORECASE | re.DOTALL))
            if matches:
        ...
        # Testemunhas: PADRÕES APRIMORADOS PARA CAPTURAR O FORMATO REAL
        testemunha_encontrada = False
        
        # Padrão 1: "1ª testemunha do reclamante: NOME, CPF X, ENDEREÇO" (formato mais comum)
        test_pattern1 = r'(\d+)ª testemunha do reclamante:\s*([A-ZÀ-Ú\s]+?)(?:,\s*CPF|$)'
        test_matches1 = list(re.finditer(test_pattern1, text, re.IGNORECASE))
        if test_matches1:
            print(f"Encontradas {len(test_matches1)} ocorrências para Testemunhas do Reclamante (padrão 1).")
            testemunha_encontrada = True
            for match in test_matches1:
                num_test = match.group(1)
                nome_testemunha = ' '.join(match.group(2).strip().split()).title()
                if nome_testemunha and nome_testemunha.strip():
                    info['testemunhas_reclamante'].append(nome_testemunha)
                    print(f"{num_test}ª Testemunha Reclamante: {nome_testemunha}")
        
        # Padrão 2: "1ª testemunha do reclamante NOME, CPF X" (sem dois pontos)
        if not test_matches1:
            test_pattern1_alt = r'(\d+)ª testemunha do reclamante\s+([A-ZÀ-Ú\s]+?)(?:,\s*CPF|$)'
            test_matches1_alt = list(re.finditer(test_pattern1_alt, text, re.IGNORECASE))
            if test_matches1_alt:
                print(f"Encontradas {len(test_matches1_alt)} ocorrências para Testemunhas do Reclamante (padrão alternativo).")
                testemunha_encontrada = True
                for match in test_matches1_alt:
                    num_test = match.group(1)
                    nome_testemunha = ' '.join(match.group(2).strip().split()).title()
                    if nome_testemunha and nome_testemunha.strip():
                        info['testemunhas_reclamante'].append(nome_testemunha)
        ...
        # Padrão 3: "1ª testemunha do(a) reclamado(a): NOME, CPF X, ENDEREÇO"
        test_pattern2 = r'(\d+)ª testemunha do\(?a?\)? reclamado\(?a?\)?\s*:\s*([A-ZÀ-Ú\s]+?)(?:,\s*CPF|$)'
        test_matches2 = list(re.finditer(test_pattern2, text, re.IGNORECASE))
        if test_matches2:
            print(f"Encontradas {len(test_matches2)} ocorrências para Testemunhas da Reclamada (padrão 1).")
            testemunha_encontrada = True
            for match in test_matches2:
                num_test = match.group(1)
                nome_testemunha = ' '.join(match.group(2).strip().split()).title()
                if nome_testemunha and nome_testemunha.strip():
                    info['testemunhas_reclamada'].append(nome_testemunha)
                    print(f"{num_test}ª Testemunha Reclamada: {nome_testemunha}")
        
        # Padrão 4: "1ª testemunha do(a) reclamado(a) NOME, CPF X" (sem dois pontos)
        if not test_matches2:
            test_pattern2_alt = r'(\d+)ª testemunha do\(?a?\)? reclamado\(?a?\)?\s+([A-ZÀ-Ú\s]+?)(?:,\s*CPF|$)'
            test_matches2_alt = list(re.finditer(test_pattern2_alt, text, re.IGNORECASE))
            if test_matches2_alt:
                print(f"Encontradas {len(test_matches2_alt)} ocorrências para Testemunhas da Reclamada (padrão alternativo).")
                testemunha_encontrada = True
                for match in test_matches2_alt:
                    num_test = match.group(1)
                    nome_testemunha = ' '.join(match.group(2).strip().split()).title()
                    if nome_testemunha and nome_testemunha.strip():
                        info['testemunhas_reclamada'].append(nome_testemunha)
        ...
        depoimento_reclamado_pattern1 = r'Depoimento\s+pessoal\s+d[oa](?:\(a\))?\s+representante\s+d[oa](?:\(a\))?\s+reclamad[ao](?:\(a\))?\s+(gravado|dispensado)'
...
def get_speaker_identification_known_values(ata_info, max_length=35):
    ...
    for prep in ata_info.get('prepostos') or []:
        if prep and isinstance(prep, str) and prep.strip() and prep.strip().upper() != "N/I":
            ...
            if s and s not in names:
                names.append(s)
    for adv in ata_info.get('advogados_reclamadas') or []:
...
def format_ata_info(ata_info):
    ...
    for i in range(num_reclamadas):
        if i < len(ata_info.get('reclamadas', [])):
            ...
            if i < len(ata_info.get('advogados_reclamadas', [])):
                formatted_parts.append(f"  → Advogado: {ata_info['advogados_reclamadas'][i]}")
            # Adiciona informação sobre o depoimento da reclamada
            if ata_info.get('depoimento_reclamado'):
                status = ata_info['depoimento_reclamado']
                if status == 'gravado':
                    formatted_parts.append(f"  → Depoimento pessoal do representante: **GRAVADO**")
                elif status == 'dispensado':
                    formatted_parts.append(f"  → Depoimento pessoal do representante: **DISPENSADO**")
    ...
    if ata_info.get('testemunhas_reclamante'):
        ...
        for i, testemunha in enumerate(ata_info['testemunhas_reclamante'], 1):
            formatted_parts.append(f"  → {i}ª Testemunha: {testemunha}")
...
def process_transcript_with_claude(transcript_path, ata_info, api_key):
    ...
    try:
        ...
        # Garante que o nome base não termine com _transcrito se já foi processado
        ...
        # Modelo recomendado atualmente (Sonnet 3.5) - rápido e capaz
...
def process_transcript_with_openai(transcript_path, ata_info, api_key):
    ...
    try:
        ...
        model_to_use = "gpt-4o-mini" # Melhor opção atual
...
def process_transcript_with_gemini(transcript_path, ata_info, api_key_gemini):
    ...
    try:
        ...
        print(f"Enviando prompt para Gemini ({model_name}) via REST API. Tamanho do texto: {len(transcript_text)} caracteres.")
        ...
        while retry_count < max_retries:
            ...
            try:
                # Timeout explícito de 900 segundos (15 minutos)
                response = requests.post(url, headers=headers, json=data, timeout=900)
                if response.status_code == 200:
                    response_json = response.json()
                    # Extrair texto da resposta JSON do Gemini
                    try:
                        response_text = response_json['candidates'][0]['content']['parts'][0]['text']
                        break # Sucesso
                    except (KeyError, IndexError):
                        print(f"Erro ao extrair texto da resposta JSON: {response_json}")
                        return f"Erro: Resposta malformada da API Gemini: {response_json}"
                else:
                    print(f"Erro na API Gemini (Status {response.status_code}): {response.text}")
                    retry_count += 1
                    time.sleep(2)
            except requests.exceptions.Timeout:
                retry_count += 1
                duration = time.time() - attempt_start
                print(f"Timeout na requisição Gemini (Tentativa {retry_count}/{max_retries}). Duração: {duration:.2f}s")
            except Exception as e:
                retry_count += 1
                duration = time.time() - attempt_start
                print(f"Erro na requisição Gemini (Tentativa {retry_count}/{max_retries}): {e}. Duração: {duration:.2f}s")
                time.sleep(2)
        
        if not response_text:
             return f"Erro persistente na API do Gemini após {max_retries} tentativas."
...
class TranscriptionWindow(QMainWindow):
    def __init__(self):
        ...
        self.service_openai = QRadioButton("Open AI (gpt-4o-transcribe)")
        self.service_gemini = QRadioButton("Gemini (2.5 Flash)") # NOVO RadioButton
        self.service_soniox = QRadioButton("Soniox (Async)")
        ...
        self.api_key_openai_input.setPlaceholderText("Necessária para Transcrição Open AI gpt-4o-transcribe ou Pós-proc. GPT")
        ...
        self.api_key_gemini_input.setPlaceholderText("Necessária para Transcrição ou Pós-proc. Gemini")
        ...
        self.use_claude_radio = QRadioButton("Claude")
        self.use_openai_radio = QRadioButton("OpenAI (GPT 4o mini)")
        self.use_gemini_radio = QRadioButton("Gemini (2.5 Flash)")
    ...
    def _get_transcription_model_tag(self, selected_service):
        ...
        mapping = {
            ...
            "Soniox": "soniox_async",
            "JustPostProcess": "local_txt"
        }
        return mapping.get(selected_service, self._sanitize_model_tag(selected_service))
    ...
    def start_transcription(self):
        ...
        openai_key = self.api_key_openai_input.text().strip()
        claude_key = self.api_key_claude_input.text().strip()
        gemini_key = self.api_key_gemini_input.text().strip()
        ...
        if self.service_gemini.isChecked() and not gemini_key:
            self.update_message_box("❌ Erro: API Key do Gemini é necessária para Transcrição Gemini.")
            return
        if self.use_gemini_radio.isChecked() and not gemini_key:
        ...
        if self.service_openai.isChecked() and not openai_key:
        ...
        if self.service_soniox.isChecked() and not soniox_key:
            ...
            return
        if self.use_openai_radio.isChecked() and not openai_key:
        ...
        if self.service_assembly.isChecked():
            selected_service = "AssemblyAI"
        elif self.service_openai.isChecked():
            selected_service = "gpt-4o-transcribe"
        elif self.service_gemini.isChecked():
            selected_service = "Gemini"
        elif self.service_soniox.isChecked():
            selected_service = "Soniox"
        elif self.service_just_post_process.isChecked():
            selected_service = "JustPostProcess"
        ...
        self.transcription_thread = threading.Thread(
            ...
            args=(audio_path, ata_path, assembly_key, openai_key, claude_key, gemini_key, soniox_key, selected_service, use_speaker_identification),
            daemon=True
        )
    ...
    def transcribe_with_gemini(self, file_path_or_url, destination_path, worker_signals, api_key_gemini):
...
Por favor, forneça uma transcrição completa e detalhada."""
            ...
            if response and hasattr(response, 'text') and response.text:
                ...
                # Salva diretamente no arquivo de destino com o nome do modelo
                base_name = os.path.splitext(destination_path)[0]
                model_suffix = model_name.replace('/', '_').replace(':', '_')
                final_path = f"{base_name}_{model_suffix}.txt"

                # Salva o texto transcrito com os timestamps originais da API Gemini
                with open(final_path, 'w', encoding='utf-8') as f:
                    f.write(transcribed_text)

                # Cria uma cópia para o arquivo principal para compatibilidade com pós-processamento
                import shutil
    ...
    def perform_transcription_workflow(self, file_path_or_url, ata_path, api_key_assembly, api_key_openai, api_key_claude, api_key_gemini, api_key_soniox, selected_service, use_speaker_identification=False):
        ...
        try:
            ...
            model_suffix = self.current_run_suffix or self._build_models_suffix(selected_service)
            ...
            if is_text_input:
                ...
                transcript_file_name = safe_base_name + "_transcrito" + model_suffix + ".txt"
            ...
            if selected_service == "AssemblyAI":
                ...
                try:
                    ...
                    if file_size > max_size_mb * 1024 * 1024:
                        ...
                        self.worker_signals.message.emit(f"🗣️ Enviando áudio para OpenAI gpt-4o-transcribe... (Tamanho: {file_size / (1024*1024):.1f}MB)")
                        ...
                        if retry_count < max_retries:  # Sucesso
                            ...
                            if response and hasattr(response, 'text') and response.text:
                                ...
                                self.worker_signals.message.emit(f"✅ Transcrição com OpenAI gpt-4o-transcribe concluída.")
                                transcript_success = True
                            else:
                     ...
                     import traceback
                     self.worker_signals.message.emit(f"❌ Erro inesperado durante a transcrição com OpenAI gpt-4o-transcribe: {str(e)}")
            ...
            if transcript_success:
                ...
                if post_processor:
                      ...
                      if post_processor == "Claude":
                           result = process_transcript_with_claude(full_destination_path, ata_info, api_key_claude)
                      elif post_processor == "OpenAI":
                           result = process_transcript_with_openai(full_destination_path, ata_info, api_key_openai)
                      elif post_processor == "Gemini":
                           result = process_transcript_with_gemini(full_destination_path, ata_info, api_key_gemini)
                ...
                if final_output_path and os.path.exists(final_output_path):
                    final_output_path = self._apply_suffix_to_output_path(final_output_path, model_suffix)
                ...
                # Remove o arquivo de transcrição sem sufixo do modelo (mantém apenas o com sufixo)
                if transcript_success and os.path.exists(full_destination_path):
                    # Verifica se existe um arquivo com sufixo do modelo correspondente
                    base_name = os.path.splitext(full_destination_path)[0]
                    model_suffix_files = [f for f in os.listdir(os.path.dirname(full_destination_path)) 
                                        if f.startswith(os.path.basename(base_name) + "_") and f.endswith(".txt")]
                    
                    if model_suffix_files:
                        try:
                            os.remove(full_destination_path)
                            self.worker_signals.message.emit(f"🗑️ Arquivo genérico removido: {os.path.basename(full_destination_path)}")
                        except Exception as e:
                            self.worker_signals.message.emit(f"⚠️ Aviso: Não foi possível remover o arquivo genérico: {e}")

                # Mensagem final com detalhes dos arquivos
                if transcript_success:
                    self.worker_signals.message.emit("\n📁 Arquivos gerados:")
                    
                    # Lista apenas os arquivos que ainda existem
                    if os.path.exists(full_destination_path):
                        ...
                        # Mostra o arquivo com sufixo do modelo
                        base_name = os.path.splitext(full_destination_path)[0]
                        model_suffix_files = [f for f in os.listdir(os.path.dirname(full_destination_path)) 
                                            if f.startswith(os.path.basename(base_name) + "_") and f.endswith(".txt")]
                        if model_suffix_files:
                            model_file_path = os.path.join(os.path.dirname(full_destination_path), model_suffix_files[0])
                            self.worker_signals.message.emit(f"1. Transcrição: {os.path.basename(model_file_path)}")
                            self.worker_signals.message.emit(f"   Local: {os.path.dirname(model_file_path)}")
